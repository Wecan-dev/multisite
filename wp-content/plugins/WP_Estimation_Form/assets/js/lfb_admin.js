...
function lfb_initWinGlobalSettings() {
    jQuery('#lfb_winGlobalSettings a[data-action="saveGlobalSettings"]').on('click', lfb_saveGlobalSettings);
...
function lfb_initFormsTopBtns() {
    jQuery('#lfb_formTopbtns a[data-action="addFormStep"]').on('click', function () {
        lfb_addStep(lfb_data.texts['My step']);
    });
    jQuery('#lfb_formTopbtns a[data-action="shortcodesInfos"]').on('click', function () {
        lfb_showShortcodeWin(lfb_currentFormID);
    });
    jQuery('#lfb_formTopbtns a[data-action="viewFormLogs"]').on('click', function () {
        lfb_loadLogs(lfb_currentFormID);
    });
    jQuery('#lfb_formTopbtns a[data-action="viewFormCharts"]').on('click', function () {
        lfb_showLoader();
        lfb_loadCharts(lfb_currentFormID);
    });
    jQuery('#lfb_formTopbtns a[data-action="viewFormVariables"]').on('click', lfb_viewFormVariables);
...
function lfb_saveForm() {
    ...
    clearInterval(lfb_canvasTimer);
    if (!lfb_disableLinksAnim) {
        lfb_canvasTimer = setInterval(lfb_updateStepCanvas, 60);
    }
    jQuery.ajax({
        url: ajaxurl,
        type: 'post',
        ...
        success: function (rep) {
           ...
           // if (!lfb_disableLinksAnim) {
                ...
                lfb_canvasTimer = setInterval(function(){                    
                     if(!lfb_disableLinksAnim || lfb_isLinking){lfb_updateStepCanvas();}
                }, 30);
           /* } else {
...
function lfb_confirmDeleteVariable() {
    ...
    $('#modal_deleteVariable').modal('hide');
    jQuery('#lfb_variablesTable tbody tr[data-id="' + variableIDableID + '"]').remove();
    jQuery.ajax({
        url: ajaxurl,
        type: 'post',
        data: {
...
function lfb_saveGlobalSettings() {
    var settingsData = {};
    var error = false;
