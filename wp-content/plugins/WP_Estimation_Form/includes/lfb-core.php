...
class LFB_Core {
    ...
    public function __construct($file = '', $version = '1.6.0') {
        ...
        //add_action('wp_ajax_nopriv_lfb_sendVerificationCode', array($this, 'sendVerificationCode'));
        //add_action('wp_ajax_lfb_sendVerificationCode', array($this, 'sendVerificationCode'));
...
// execute url variables
                ...
                if (count($formReq) > 0) {
                    ...
                    if ($form->use_razorpay || $form->use_stripe || ($form->use_paypal && $form->paypal_useIpn)) {
                        ...
                        if ($form->use_stripe) {
                            ...
                            $response .= '<p style="margin-top: 38px; margin-bottom: -28px;" class="lfb_btnNextContainer"><a href="javascript:" id="lfb_btnPayStripe"  class="btn btn-wide btn-primary">' . $form->last_btn . '</a></p>';
                        }
                        if ($form->use_paypal) {
    ...
    public function forgotPassManD() {
        ...
        if (!isset($_SESSION['lfb_lastPassSent']) || $_SESSION['lfb_lastPassSent'] != $email) {
            $_SESSION['lfb_lastPassSent'] = $email;
    ...
    public function showCustomerManagementPage($pageContent) {
        ...
        if ($settings->enableCustomerAccount && get_the_ID() == $settings->customerAccountPageID) {
            ...
            if (isset($_SESSION['lfb_loginMan']) && $_SESSION['lfb_loginMan'] != '0') {
                $customerID = intval($_SESSION['lfb_loginMan']);
                global $wpdb;
    ...
    public function frontend_enqueue_styles($hook = '') {
        ...
        $_SESSION['lfb_activationCode'] = '';
        $_SESSION['lfb_verifiedEmail'] = '';
...
// $pos_end=strpos($post->post_content, '"', strpos($post->post_content, 'form_id="', strpos($post->post_content, '[estimation_form') + 16) + 10)-1;
    ...
    /* public function checkCaptcha() {
      ...
      $table_name = $wpdb->prefix . "wpefc_forms";
      $rows = $wpdb->get_results($wpdb->prepare("SELECT id,recaptcha3KeySecret FROM $table_name WHERE id=%s", $formID));
      if (count($rows) > 0) {
      $form = $rows[0];
      ...
      public function checkCaptchaBak() {
      $captcha = sanitize_text_field($_POST['captcha']);
      if (session_status() == PHP_SESSION_NONE) { session_start(); }
...
//$form=$formReq->form_page_id;
                ...
                $response .= '<div id="lfb_stripeModal" class="modal" data-backdrop="">'
                        ...
                        . '</form>'
                        . '<div class="text-center lfb_stripeLogoContainer"><img class="lfb_stripeLogo" src="' . $this->assets_url . 'img/powered_by_stripe@2x.png" alt="Powered by Stripe" /></div>'
                ...
                if ($form->use_stripe && $form->paymentType != "email") {
                    ...
                    $response .= '<p style="margin-top: 38px; margin-bottom: -28px;" class="lfb_btnNextContainerStripe">';
    ...
    public function frontend_enqueue_scripts($hook = '') {
        ...
        if (($settings->enableCustomerAccount && $settings->customerAccountPageID > 0 && get_the_ID() == $settings->customerAccountPageID)) {
            ...
            $customerID = 0;
            if (isset($_SESSION['lfb_loginMan']) && $_SESSION['lfb_loginMan'] != '0') {
                $chkCustomerID = intval($_SESSION['lfb_loginMan']);
                ...
                if (count($customerData) > 0) {
                    $customerID = $_SESSION['lfb_loginMan'];
                }
            }
            wp_localize_script($this->_token . '-accountManagement', 'lfb_dataCust', array(
                ...
                'customersAc_sendPass' => $settings->customersAc_sendPass,
    ...
    private function lfb_generatePdfCustomer($order, $form, $fromCustomer) {
        ...
        if ($form) {
            ...
            $_SESSION['orderToDownload'] = $uniqueID;
            $_SESSION['orderTitle'] = $order->ref;
    ...
    private function sendOrderEmail($orderRef, $formID) {
        ...
        if (count($rows) > 0) {
            ...
            /* if (strpos($order->contentUser, '[gdpr_link]') !== false) {
              ...
              $customersDataEmailLink = str_replace("[url]",$customersDataUrl);
    ...
    public function send_email() {
        ...
        $this->verifyCaptchaThen($form,
            ...
            if ($settings->enableCustomerAccount) {
                ...
                if (!$chkEmail && isset($_SESSION['lfb_verifiedEmail']) && $_SESSION['lfb_verifiedEmail'] == $verifiedEmail) {
                    $chkEmail = true;
                }
            }
            if ($chkEmail) {
...
//$contentProject = str_replace("<table",'<table style="table-display: block;"', $contentProject);
                ...
                if (!isset($gravity) || $gravity == 0) {
                    ...
                    if ($emailToUser == '1') {
                        ...
                        while (($lastPos = strpos($projectCustomer, '<span class="lfb_value">', $lastPos)) !== false) {
                            ...
                            $lastPos = $lastPos + strlen('<span class="lfb_value">');
                            $fileStartPos = $lastPos;
// $fileStartPos = strpos($projectCustomer, ':', $lastPos) + 2;
...
// user email
                        ...
                        $content = $this->prepareEmailContent($content, $form, $formSession, $variables, $itemsArray);
...
// $projectAdmin = str_replace("<table",'<table style="table-display: block;"', $projectAdmin);
                    $lastPos = 0;
                    $positions = array();
                    ...
                    while (($lastPos = strpos($projectAdmin, '<span class="lfb_value">', $lastPos)) !== false) {
                        $positions[] = $lastPos;
                        $lastPos = $lastPos + strlen('<span class="lfb_value">');
                        $fileStartPos = $lastPos;
// $fileStartPos = strpos($projectAdmin, ':', $lastPos) + 2;
                        $lastSpan = strpos($projectAdmin, '</span>', $fileStartPos);
                        $value = substr($projectAdmin, $fileStartPos, $lastSpan - $fileStartPos);
                        $toReplaceDefault[] = '<span class="lfb_value">' . $value . '</span>';
                        $toReplaceBy[] = '<span class="lfb_value">' . $this->stringEncode($value, $settings->encryptDB) . '</span>';
                    }
                    foreach ($toReplaceBy as $key => $value) {
...
// admin email
                    ...
                    $contentAdmin = $this->prepareEmailContent($content, $form, $formSession, $variables, $itemsArray);
...
// user pdf
                    ...
                    if (isset($email) && $contactSent == 0) {
                        if ($form->useMailchimp && $form->mailchimpList != "") {
                            try {
                                ...
                                $merge_vars = array('FNAME' => $customerInfos['firstName'], 'LNAME' => $customerInfos['lastName'], 'phone' => $customerInfos['phone'],
                                    'address1' => array('addr1' => $customerInfos['address'], 'city' => $customerInfos['city'], 'state' => $customerInfos['state'], 'zip' => $customerInfos['zip'], 'country' => $customerInfos['country']));
                        ...
                        if ($form->useGetResponse) {
                            ...
                            $merge_vars = array('phone' => $customerInfos['phone'], 'city' => $customerInfos['city'], 'state' => $customerInfos['state'], 'postal_code' => $customerInfos['zip'], 'country' => $customerInfos['country']);
                            $GetResponse->addContact($form->getResponseList, $customerInfos['firstName'] . ' ' . $customerInfos['lastName'], $email, 'standard', 0, $merge_vars);
                        }
                    }
                    $table_name = $wpdb->prefix . "wpefc_customers";
                    ...
                    if ($customerID == 0) {
                        ...
                        $wpdb->insert($table_name, array(
                            ...
                            'phone' => $this->stringEncode($customerInfos['phone'], $settings->encryptDB), 'firstName' => $this->stringEncode($customerInfos['firstName'], $settings->encryptDB), 'lastName' => $this->stringEncode($customerInfos['lastName'], $settings->encryptDB),
    ...
    private function prepareEmailContent($content, $form, $formSession, $variables, $itemsArray) {
...
//variation
    ...
    public function makeRazorPayment() {
        ...
        if ($subCost > 0) {
            ...
            try {
                ...
                $subData = array('plan_id' => $plan->id, 'notes' => ['email' => $email], 'total_count' => $maxCount);
    ...
    public function checkEmailCustomer() {
        ...
        foreach ($customersData as $customerData) {
            if ($this->stringDecode($customerData->email, $settings->encryptDB) == $email) {
                $_SESSION['lfb_proposedEmail'] = $email;
                echo 1;
                break;
            }
        }
    ...
    public function sendVerificationCode() {
        ...
        $_SESSION['lfb_activationCode'] = $this->generateRandomString(4);
        $_SESSION['lfb_verifiedEmail'] = $email;

        $table_name = $wpdb->prefix . "wpefc_forms";
        $formReq = $wpdb->get_results($wpdb->prepare("SELECT * FROM $table_name WHERE id='%s' LIMIT 1", $formID));
        if (count($formReq) > 0) {
            $form = $formReq[0];
            $txtMail = $form->emailVerificationContent;
            $txtMail = str_replace('[code]', $_SESSION['lfb_activationCode'], $txtMail);
            $headers = "";
            if ($form->email_name != "") {
                global $_currentFormID;
                $_currentFormID = $formID;
            ...
            if (wp_mail($email, $form->emailVerificationSubject, $txtMail)) {
                echo 1;
            }
        }
        die();
    ...
    public function verificationPass() {
        ...
        if (isset($_SESSION['lfb_proposedEmail'])) {
            $email = $_SESSION['lfb_proposedEmail'];
            $settings = $this->getSettings();
    ...
    public function verificationCode() {
        ...
        if (isset($_SESSION['lfb_activationCode']) && $_SESSION['lfb_activationCode'] == $code) {
            echo 1;
        }
        die();
    }
    ...
    public function loadCustomerOrders() {
        ...
        if (isset($_SESSION['lfb_loginMan']) && intval($_SESSION['lfb_loginMan']) > 0) {
            $customerID = intval($_SESSION['lfb_loginMan']);
            $table_name = $wpdb->prefix . "wpefc_logs";
            $ordersData = array();
    ...
    public function saveCustomerInfos() {
        ...
        if (isset($_SESSION['lfb_loginMan']) && intval($_SESSION['lfb_loginMan']) > 0) {
            ...
            $sqlData = array(
                ...
                'lastName' => sanitize_text_field($_POST['lastName']),
                'phone' => sanitize_text_field($_POST['phone']),
                'phoneJob' => sanitize_text_field($_POST['phoneJob']),
    ...
    public function downloadCustomerOrder() {
        ...
        if (isset($_SESSION['lfb_loginMan']) && intval($_SESSION['lfb_loginMan']) > 0) {
            ...
            if (count($orders) > 0) {
                ...
                $order->contentUser = $this->stringDecode($order->contentUser, $settings->encryptDB);
                if ($order->contentUser != '') {
                    $file = $this->lfb_generatePdfCustomer($order, $form, true);
                    echo 1;
                }
            }
        }
    ...
    public function downloadMyOrder() {
        ...
        if (isset($_SESSION['orderToDownload']) && isset($_SESSION['orderTitle'])) {
            if (file_exists($this->dir . '/uploads/' . $_SESSION['orderToDownload'] . '.pdf')) {

                $fileName = $_SESSION['orderToDownload'] . '.pdf';
                $target_path = plugin_dir_path(__FILE__) . '../uploads/' . $fileName;
                header('Content-type: application/pdf');
                header('Content-Disposition: attachment; filename="' . $_SESSION['orderTitle'] . '.pdf"');
