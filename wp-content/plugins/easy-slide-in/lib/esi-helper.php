...
function esi_get_image ($post_id=false, $size='medium') {
	...
	if ($thumb_id) {
		...
		if ($image) return apply_filters(
			'esi-media-image',
			apply_filters('esi-media-image-featured_image', $image[0], $size), $size
		);
	}
 ...
 */
function esi_get_description ($post_id=false) {
	// If we don't have post id, no reason to even try.
	$post_id = (int)$post_id;
	if (!$post_id) return apply_filters(
		'esi-media-title', get_bloginfo('name')
	);

	return apply_filters(
		'esi-media-title',
		apply_filters('esi-media-title-post_title', get_the_title($post_id))
	);
}
 ...
 */
function esi_get_url ($post_id=false) {
	...
	return apply_filters(
		'esi-media-url',
		apply_filters('esi-media-url-post_url', get_permalink($post_id))
	);
}
 ...
 */
function esi_get_related_posts ($post_id, $taxonomy, $limit=3) {
	$post_id = (int)$post_id;
	if (!$post_id) return apply_filters(
		'esi-media-related_posts', array()
	);

	if ($taxonomy == 'esinextpost') {
		$next_post = get_next_post();
		$query = new stdClass();
		$query->posts = array();
		$query->posts[] = $next_post;
	}
	else {
	...
	return apply_filters(
		'esi-media-related_posts',
		apply_filters('esi-media-related_posts-posts', $query->posts)
	);
}
 ...
 */
function esi_get_related_post_excerpt ($post) {
	...
	$string = (preg_match('/.{156,}/um', $string))
		...
		: $string
	;
	return $string;// . '&nbsp;' . '<a href="' . get_permalink($post->ID) . '">' . __('Read more', 'esi') . '<a>';
}

function esi_get_related_post_thumbnail ($post_id) {
 ...
 */
function esi_getval ($from, $what, $default=false) {
