...
if ( ! function_exists( 'wc_api_mps_add_admin_menu' ) ) {
    ...
    function wc_api_mps_add_admin_menu() {
        ...
        add_submenu_page( 'wc_api_mps', 'Product Sync - API Error Logs', 'API Error Logs', 'manage_options', 'wc_api_mps_api_error_logs', 'wc_api_mps_api_error_logs_callback' );
...
if ( ! function_exists( 'wc_api_mps_callback' ) ) {
    function wc_api_mps_callback() {
        ...
        if ( isset( $_REQUEST['submit'] ) ) {
            ...
            $stores = get_option( 'wc_api_mps_stores' );
            unset( $stores[base64_decode( $_REQUEST['delete'] )] );
            update_option( 'wc_api_mps_stores', $stores );
            ?>
                <div class="notice notice-success is-dismissible">
                ...
                <?php
                    ...
                    if ( $licence ) {
                        if ( isset( $_REQUEST['edit'] ) ) {
                            ...
                            $stores = get_option( 'wc_api_mps_stores' );
                            $store = $stores[base64_decode( $_REQUEST['edit'] )];
                            ?>
                                <h2><?php _e( 'Edit store:' ); ?> <?php echo base64_decode( $_REQUEST['edit'] ); ?></h2>                           
                                <form method="post" action="<?php echo $page_url; ?>">
                                    ...
                                    <p>
                                        <input type="hidden" name="url" value="<?php echo base64_decode( $_REQUEST['edit'] ); ?>" />
                                                ...
                                                foreach ( $stores as $store => $data ) {
                                                    ?>
                                                        <tr>
                                                            ...
                                                            <td>
                                                                <a href="<?php echo $page_url; ?>&edit=<?php echo base64_encode( $store ); ?>"><span class="dashicons dashicons-edit"></span></a>
                                                                <a href="<?php echo $page_url; ?>&delete=<?php echo base64_encode( $store ); ?>"><span class="dashicons dashicons-trash"></span></a>
                                                            </td>
                                                        </tr>
                                                    <?php
                                                }
    ...
    function wc_api_mps_bulk_sync_callback() {
        ...
        if ( isset( $_REQUEST['submit'] ) ) {
            ...
            if ( $records != null ) {
                ...
                ?>
                    <div class="notice notice-success is-dismissible">
                        <p><?php _e( 'Products successfully synced.' ); ?></p>
                    </div>
                <?php
            }
        }
                    ...
                    if ( $licence ) {
                        ...
                        <form method="post" action="<?php echo menu_page_url( 'wc_zoho_integration', 0 ); ?>">                
                            <table class="form-table">
                                <tbody>
                                    <tr>
                                        <th scope="row"><?php _e( 'Search products' ); ?></th>
                            ...
                            <p class="submit">
                                <input name="filter" class="button button-secondary" value="<?php _e( 'Filter' ); ?>" type="submit">
                        ...
                        <form method="post">                
                            <table class="form-table">
                                <tbody>
                                    ...
                                    <tr>
                                        <th scope="row"><?php _e( 'Product Sync Type' ); ?></th>
                                        <td>
                                            <fieldset>
                                                <label><input type="radio" name="wc_api_mps_product_sync_type" value="full_product"<?php echo ( $product_sync_type == 'full_product' ? ' checked="checked"' : '' ); ?> /> <?php _e( 'Full Product' ); ?></label><br>
                                                <label><input type="radio" name="wc_api_mps_product_sync_type" value="price_and_quantity"<?php echo ( $product_sync_type == 'price_and_quantity' ? ' checked="checked"' : '' ); ?> /> <?php _e( 'Price and Quantity' ); ?></label><br>
                                                <label><input type="radio" name="wc_api_mps_product_sync_type" value="quantity"<?php echo ( $product_sync_type == 'quantity' ? ' checked="checked"' : '' ); ?> /> <?php _e( 'Quantity' ); ?></label>
                                            </fieldset>
                                        </td>
                                    </tr>
                                    <tr style="<?php echo ( $product_sync_type != 'full_product' ? 'display: none;' : '' ); ?>">
                                    ...
                                    <tr>
                                        <th scope="row"><?php _e( 'Sync on product delete?' ); ?></th>
                                        <td>
                            ...
                            if ( $file_size ) {
                                ...
                                if ( $file_data ) {
                                    ...
                                    ?>
                                        <form method="post">
                                            <p>
                                                <input type='submit' class='button-primary' name="submit" value="<?php _e( 'Clear API Error Logs' ); ?>" />
                                            </p>
                                        </form>
                                    <?php
                                }
                            } else {
                                ?><p><?php _e( 'No API error logs found.' ); ?></p><?php
                            }
                        fclose( $file );
                    } else {
                        ?>
